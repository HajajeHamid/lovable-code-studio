...
export class TPParser {
  ...
  private calculateStatistics(
    ...
    return {
      ...
      enums: program.enums?.length || 0,
      dataJsons: program.dataJsons?.length || 0,
      models: program.models?.length || 0,
      components: program.components?.length || 0,
      pages: program.pages?.length || 0,
      apis: program.apis?.length || 0,
      microservices: program.microservices?.length || 0,
      imports: program.imports?.length || 0,
      autoGens: program.autoGens?.length || 0,
      deploys: program.deploys?.length || 0,
      tests: program.tests?.length || 0,
      integrations: program.integrations?.length || 0,
      sagas: program.sagas?.length || 0,
      businessRules: program.businessRules?.length || 0,
      parseTime,
    };
  }
